  ...
  var retfunc = function() {
    ...
    PlotAxis.prototype.setRange = function(vl, vr, para) {
      ...
      if (this.axisValR instanceof Big) {
        this.axisValSpan = this.axisValR.minus(this.axisValL);
      } else {
    ...
    PlotAxis.prototype.setGridlines = function(pl, pr, count, ml, mr) {
      ...
      if (this.axisType === "nanotime") {
        this.axisMarginValL = this.axisValSpan.times(ml);
        this.axisMarginValR = this.axisValSpan.times(mr);
      }
      ...
      if (this.axisValSpan instanceof Big) {
        span = this.axisValSpan.minus(this.axisMarginValL.plus(this.axisMarginValR)).times(this.axisPctSpan);
      } else {
        span = (this.axisValSpan - (this.axisMarginValL + this.axisMarginValR)) * this.axisPctSpan;
      }
    ...
    PlotAxis.prototype.calcLabels = function (lines, span, axisType) {
      ...
      if ((span > this.SECOND && axisType === "time" || axisType === "nanotime" && span > this.UNIT) && labels.length != _.uniq(labels).length) {
        if (axisType === "nanotime" && span <= this.SECOND){
          span = this.UNIT - 1;
        } else if (span <= this.MINUTE) {
          span = this.SECOND - 1;
        } else if (span <= this.HOUR) {
          span = this.MINUTE - 1;
        } else if (span <= this.DAY) {
          span = this.HOUR - 1;
        } else if (span <= this.MONTH) {
          span = this.DAY - 1;
        } else if (span <= this.YEAR) {
          span = this.MONTH - 1;
        } else {
          span = this.YEAR - 1;
        }
      ...
      var calcCommonPart = function () {
        ...
        if ((axisType === "time" || axisType === "nanotime") && span >= self.HOUR && labels.length > 1) {
    ...
    PlotAxis.prototype.getValue = function(pct) {
      ...
      if (pct > 1) { pct = 1; }
      if (this.axisValSpan instanceof Big) {
        return this.axisValSpan.times(pct).plus(this.axisValL);
      } else {
