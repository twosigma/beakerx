  ...
  function createScope(wrapperId) {
    ...
    scope.initLayout = function() {
      ...
      if(model.customStyles) {
        ...
        $(styleString).prependTo(element.find('.plot-plotcontainer'));
      }
      ...
      scope.watchModelGetWidth = function(newWidth) {
      ...
      scope.jqcontainer.on('resize', function(e, ui) {
        scope.watchModelGetWidth()
      });
    ...
    scope.initZoom = function() {
      ...
      scope.svg.on("dblclick", function() {
    ...
    scope.init = function() {
      ...
      if (!scope.model.disableContextMenu) {
        $.contextMenu({
          selector: 'div#'+scope.wrapperId+' #' + scope.id,
          zIndex: 3,
    ...
    scope.setModelData = function(data) {
      ...
      element = $('div#'+scope.wrapperId+' .dtcontainer');
    ...
    scope.fillCellModelWithPlotMethods = function () {
      var model = scope.model.getCellModel();
      if(scope.modelHasPlotSpecificMethods(model)) {
        return;
      }
