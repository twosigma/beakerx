...
public class CommandProcessorImpl implements CommandProcessor {
  ...
  private List<CommandCode> buildCommandsMap(Code code) {
    ...
    for (String lineOfCode: code.asString().split("\\r\\n|\\n|\\r")) {
      ...
      if (isThatMagic(lineOfCode)) {
        commandsToExecute.add(getMagicCommandAndCode(lineOfCode));
  ...
  private CommandCode getMagicCommandAndCode(String code) {
    for (MagicCommand currentMagicCommand : availableCommands) {
      ...
      if (isThatCorrect) {
        return new CommandCode(currentMagicCommand, code.replace(currentMagicCommand.getName(), "").trim());
      }
    }
