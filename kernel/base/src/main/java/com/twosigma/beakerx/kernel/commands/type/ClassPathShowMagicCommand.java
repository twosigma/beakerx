...
public class ClassPathShowMagicCommand extends ClassPathMagicCommand {
  ...
  public ClassPathShowMagicCommand(KernelFunctionality kernel, MessageCreator messageCreator) {
    super(CLASSPATH_SHOW, "", Sets.newHashSet(MagicCommandType.LINE), messageCreator, kernel);
  }
  ...
  public MagicCommandFunctionality build() {
    return (code, message, executionCount) -> {
      ...
      return new CommandItemWithResult(getMessageCreator().buildOutputMessage(message, result.getData().toString(), false),
