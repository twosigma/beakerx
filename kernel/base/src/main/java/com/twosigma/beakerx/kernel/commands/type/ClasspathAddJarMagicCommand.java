...
public class ClasspathAddJarMagicCommand extends ClassPathMagicCommand {
  ...
  public ClasspathAddJarMagicCommand(KernelFunctionality kernel, MessageCreator messageCreator) {
    super(CLASSPATH_ADD_JAR, "<jar path>", Sets.newHashSet(MagicCommandType.LINE), messageCreator, kernel);
  }

  @Override
  public MagicCommandFunctionality build() {
    return (code, message, executionCount) -> {
      ErrorData errorData = isValidPath(code);

      if (errorData.hasError()) {
        return createErrorMessage(message, errorData.getMessage(), executionCount);
      } else {
        return getMagicCommandItem(addJars(code), code, message, executionCount);
      }
    };
  }
}
