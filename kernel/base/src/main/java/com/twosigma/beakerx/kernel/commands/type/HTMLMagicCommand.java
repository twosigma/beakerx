...
public class HTMLMagicCommand extends MagicCommand {

  public HTMLMagicCommand(MessageCreator messageCreator) {
    super(HTML, "", Sets.newHashSet(MagicCommandType.CELL), messageCreator);
  }

  @Override
  public MagicCommandFunctionality build() {
    return (code, message, executionCount) -> {
            MIMEContainer html = HTML("<html>" + code + "</html>");
            return new CommandItemWithResult(getMessageCreator().buildMessage(message, singletonList(html), executionCount),
