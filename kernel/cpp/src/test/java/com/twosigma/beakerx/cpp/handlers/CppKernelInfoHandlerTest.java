...
public class CppKernelInfoHandlerTest {
  ...
  private static CppKernelFunctionalityTest kernel;
  private static CppKernelInfoHandler handler;

  @BeforeClass
  public static void setUpClass() throws Exception {
    kernel = new CppKernelFunctionalityTest();
  ...
  @Test
  public void handle_messageContentHasCppLabel() throws Exception {
    //when
    handler.handle(message);
    //then
    Message sentMessage = kernel.getSentMessages().get(0);
    Map<String, Serializable> map = sentMessage.getContent();
    Assertions.assertThat(map).isNotNull();
    Assertions.assertThat(map.get("implementation")).isEqualTo("cpp");
  }
