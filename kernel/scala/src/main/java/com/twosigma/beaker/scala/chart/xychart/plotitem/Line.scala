...
class Line(var xs: List[AnyVal], var ys: List[AnyVal]) extends com.twosigma.beaker.chart.xychart.plotitem.Line(xs.map(x => x.asInstanceOf[AnyRef]).asJava, ys.map(x => x.asInstanceOf[Number]).asJava) {
