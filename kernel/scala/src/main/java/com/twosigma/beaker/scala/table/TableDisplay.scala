 ...
 */
package com.twosigma.beaker.scala.table

import java.util.concurrent.TimeUnit

import com.twosigma.beaker.jvm.serialization.BeakerObjectConverter
import com.twosigma.beaker.table._
...
object TableDisplay {
  ...
  private def create(v: Map[_,_]) = {
    new com.twosigma.beaker.table.TableDisplay(mapAsJavaMap(v))
  }
  ...
  private def create(v: List[List[_]], co: List[String], cl: List[String]) = {
    ...
    new com.twosigma.beaker.table.TableDisplay(javaListOfList, co.asJava, cl.asJava)
  }

  private def create(v: Array[Map[_, _]]): com.twosigma.beaker.table.TableDisplay = {
    val javaStandardized : Array[java.util.Map[_, _]]= v.map(v => mapAsJavaMap(v)).toArray

    new com.twosigma.beaker.table.TableDisplay(javaStandardized)
  }

  private def create(v: List[Map[_, _]]): com.twosigma.beaker.table.TableDisplay = {
    ...
    new com.twosigma.beaker.table.TableDisplay(javaCollection)
  }

  private def create(v: List[Map[_, _]], serializer: BeakerObjectConverter): com.twosigma.beaker.table.TableDisplay = {
    val javaMaps : List[java.util.Map[_,_]] = v.map(entry => mapAsJavaMap(entry))
    val javaCollection : java.util.Collection[java.util.Map[_,_]] = javaMaps.asJavaCollection

    new com.twosigma.beaker.table.TableDisplay(javaCollection, serializer)
  }

}

class TableDisplay private (tableDisplay: com.twosigma.beaker.table.TableDisplay) {
  def this(v: Map[_,_]) = {
