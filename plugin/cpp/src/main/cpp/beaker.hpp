...
jobject convert(std::map<T1, T2> theMap){
...
// Unconvert
int unconvert(jobject obj, int &ref){
  jclass typeClass = globEnv->FindClass("java/lang/Integer");
...
int unconvert(jobject obj, bool &ref){
  jclass typeClass = globEnv->FindClass("java/lang/Boolean");
...
int unconvert(jobject obj, float &ref){
  jclass typeClass = globEnv->FindClass("java/lang/Float");
...
int unconvert(jobject obj, double &ref){
  jclass typeClass = globEnv->FindClass("java/lang/Double");
...
int unconvert(jobject obj, std::string &ref){
  jstring jstr = (jstring) obj;
  const char* str = globEnv->GetStringUTFChars(jstr, NULL);
  ref = std::string(str);
  return 1;
}
int unconvert(jobject obj, char* ref){
  jstring jstr = (jstring) obj;
  const char* str = globEnv->GetStringUTFChars(jstr, NULL);
  ref = strdup(str);
  return 1;
...
template <typename T>
int unconvert(jobject obj, std::vector<T> &ref){
  ref = std::vector<T>();
  // Get array list size
...
// Javascript arrays become java arraylists
template <typename T1, typename T2>
int unconvert(jobject jmap, std::map<T1, T2> &ref){
  ref = std::map<T1, T2>();
  // Get array list size
...
// Beaker object
template <typename T>
int beakerGet(std::string name, T &ret) {
...
template <typename T>
int beakerSet(std::string name, T val){
  jstring jname = globEnv->NewStringUTF(name.c_str());
