 ...
 */
/**
 * Python3 eval plugin For creating and config evaluators that uses a IPython
 * kernel for evaluating python3 code and updating code cell outputs.
 */
define(function(require, exports, module) {
  ...
  var Python3Proto = {
            ...
            if (ipyVersion == '1') {
              ...
              var ajaxsettings = {
                ...
                success : function(data, status, xhr) {
                  ...
                  if (ipyVersion == '2') {
                    self.kernel._kernel_started({
                      id : data.kernel.id
                    });
                  } else {
                    self.kernel._kernel_created({
                      id : data.kernel.id
                    });
      ...
      var output = function output(a0, a1) {
        ...
        if (isIpythonVersion3OrAbove() ? (type === "error") : (type === "pyerr")) {
          ...
          evaluation.outputdata.push({
            type : (content.name === "stderr") ? 'err' : 'out',
            value : text
          });
        } else {
          var jsonres;
          ...
          if (jsonres !== undefined && _.isObject(jsonres) && jsonres.type !== undefined) {
            if (finalStuff !== undefined && finalStuff.payload !== undefined) {
              // if we already received an output we should append this output
              // to it
            ...
            var oa = isIpythonVersion3OrAbove() ? (new myPython.OutputArea({
              events : {
                trigger : function() {
                }
              },
    ...
    autocomplete : function(code, cpos, cb) {
      ...
      if (ipyVersion == '1') {
        return kernel.complete(code, cpos, {
          'complete_reply' : function(reply) {
    ...
    reset : function() {
      ...
      kernel.restart(function() {
        var waitForKernel = function() {
          if (isIpythonVersion3OrAbove() ? (kernel.ws.readyState == 1) : (kernel.shell_channel.readyState == 1 && kernel.stdin_channel.readyState == 1 && kernel.iopub_channel.readyState == 1)) {
            ...
            var finish = function() {
              if (bkHelper.hasSessionId() && isNewShell) {
                self.evaluate(self.initCode(), {}).then(function() {
                  ...
                  var errorHtml = 'See <a target="_blank" href="https://github.com/twosigma/beaker-notebook/wiki/Python-Mismatch-Errors">our wiki</a> for how to handle this.';
