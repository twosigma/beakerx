...
define('jupyter_js_widgets_manager_v1_2_0', [
  'jupyter_js_widgets_embed_helper_v1_2_0',
  'jupyter_js_widgets_utils_v1_2_0',
  'jupyter_js_widgets_manager_base_v1_2_0',
  'jupyter_js_widgets_progress_modal_v1_2_0',
  'backbone',
  'html2canvas'
], function (embedHelper, utils, managerBase, progressModal, Backbone, html2canvas) {
"use strict";
  ...
  managerBase.ManagerBase.apply(this);
    ...
    this.notebook.events.on('before_save.Notebook', (function() {
        ...
        if (save_callback) {
            ...
            }).bind(this)).catch(utils.reject('Could not call widget save state callback.', true));
        }
    }).bind(this));
...
WidgetManager.set_state_callbacks(function() {
    ...
    Jupyter.notebook.metadata.widgets = { //MODIFIRED
    //    state: stateToSave,
...
WidgetManager.prototype._init_menu = function() {
    ...
    //widgetsSubmenu.appendChild(this._createMenuItem('Save notebook with snapshots', this.buildSnapshotsAction)); // disabled in beaker app
    //widgetsSubmenu.appendChild(this._createMenuItem('Download widget state', saveState.action)); // disabled in beaker app
    //widgetsSubmenu.appendChild(this._createMenuItem('Embed widgets', embedWidgets.action)); // disabled in beaker app
};
...
WidgetManager.prototype.display_view = function(msg, view, options) {
    if (options.cell === null) {
      ...
      if (true){
            var that = this;
            return this.display_widget_view(msg, Promise.resolve(view)).then(function(view) {
...
//MODIFIED (NOT PRESENT IN ORIGINAL) !!!
...
WidgetManager.prototype.updateSnapshots = function() {
    ...
    }).catch(utils.reject('Could not create widget snapshots and save the notebook', true));
};
...
WidgetManager.prototype._updateCellSnapshots = function(cell, index) {
    ...
    return that.progressModal.setText(
        ...
        bundle['text/html'] = embedHelper.generateEmbedScript(widgetState, imageDataUrl);
...
WidgetManager.prototype._getCellWidgetStates = function(cell, index) {
    ...
    return Promise.all(modelIds.map(function(modelId) {
        return that._models[modelId].then(function(model) {
            return utils.resolvePromisesDict(model.views).then(function(views) {
...
WidgetManager.prototype._traverseWidgetState = function(state) { //TODO ???
    var that = this;
