    ...
    for kernel in kernels:        
        ...
        try:
            if kernel == 'groovy':
                ...
                p.expect(r'Available cell magics:', timeout=t)
                p.expect(r'%%!', timeout=t)
                p.expect(r'%%HTML', timeout=t)
                p.expect(r'%%SVG', timeout=t)
                p.expect(r'%%bash', timeout=t)
                p.expect(r'%%capture', timeout=t)
                p.expect(r'%%debug', timeout=t)
                p.expect(r'%%file', timeout=t)
                p.expect(r'%%html', timeout=t) 
                p.expect(r'%%javascript', timeout=t)
                p.expect(r'%%js', timeout=t)
                p.expect(r'%%latex', timeout=t)
                p.expect(r'%%markdown', timeout=t) 
                p.expect(r'%%perl', timeout=t) 
                p.expect(r'%prun', timeout=t) 
                p.expect(r'%%pypy', timeout=t)
                p.expect(r'%%python', timeout=t)
                p.expect(r'%%python2', timeout=t)
                p.expect(r'%%python3', timeout=t)
                p.expect(r'%%ruby', timeout=t)
                p.expect(r'%%script', timeout=t) 
                p.expect(r'%%sh', timeout=t)
                p.expect(r'%%svg', timeout=t)
                p.expect(r'%%sx', timeout=t)
                p.expect(r'%%system', timeout=t) 
                p.expect(r'%%time', timeout=t) 
                p.expect(r'%%timeit', timeout=t) 
                p.expect(r'%%writefile', timeout=t)
