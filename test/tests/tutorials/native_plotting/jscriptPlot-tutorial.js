...
describe('Native Plotting in JavaScript', function () {

    beforeAll(function(done){
        beakerPO = new BeakerPageObject();
        browser.get(beakerPO.baseURL + "beaker/#/open?uri=file:config%2Ftutorials%2Fplotting-js-api.bkr&readOnly=true").then(done);
        beakerPO.waitUntilLoadingFinished();
    });

    afterAll(function(done){
        beakerPO.createScreenshot('jscriptPlotTutorial');
        done();
    });

    it('Plot with simple properties', function () {
        var idCell = "codeoTTuqE";
    ...
    it('Save plot as SVG/PNG', function() {
        beakerPO.checkSaveAsSvgPngByIdCell("codeoTTuqE", "Title");
    })
